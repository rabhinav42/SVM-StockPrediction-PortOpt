...
for(i in 1:length(companies)){                  ## set up data for relevant companies
...
port.ga <- (optimal_returns_comp)
port.rand <- (retports)
port.windmark <- (rets1)
port.mark <- (portrets)

c(length(index(port.ga)) , length(index(port.rand)) , length(index(port.windmark)) , length(index(port.mark)))
common.period <- index(port.rand)

port.ga <- port.ga[common.period]
port.rand <- port.rand[common.period]
port.windmark <- port.windmark[common.period]
port.mark <- port.mark[common.period]
...
plot(cbind(cumprod(port.ga)[common.period] , cumprod(port.rand)[common.period] , cumprod(port.windmark)[common.period] , cumprod(port.mark)[common.period] , cumprod(Nifty)[common.period]) , col = c("red" , "blue" , "brown" , "violet" , "black"))
